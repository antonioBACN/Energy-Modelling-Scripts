/*
This script is designed to automate the creation of NECB System 3 (single-zone packaged rooftop unit with baseboard heating) directly on the IDF file generated by DesignBuilder, 
starting from the HVAC systems defined in the proposed building model.

NECB System 3 is typically applied to commercial applications, including light commercial buildings, which are commonly conditioned using rooftop units. Preparing a reference model 
from the proposed model is an iterative and manual process, which involves adjusting HVAC parameters according to Part 8 of the NECB. This script automates all these steps, reducing
manual work, avoiding errors, and ensuring consistency across models.

How the Script Works:

1) Reference Model Initialization
    * The script starts from the proposed building model with all HVAC systems already defined.
    * It modifies the IDF file to create a reference model, adjusting parameters manually according to Part 8 of the NECB.
2) Night Cycle and Fan Settings
    * Automatically disables the Night Cycle in rooftop units.
    * Configures the fans of rooftop units with:
        Static pressure: 640 Pa
        Combined fan-motor efficiency: 0.4
3) Hot Water Plant Configuration
    * Creates and configures the hot water plant according to NECB requirements.
    * Ensures proper integration with baseboards and heating schedules.
4) Baseboard Assignment
    * Assigns baseboards to each thermal zone.
    * Applies appropriate schedules for each baseboard unit.
5) DX Cooling Coils Adjustment
    * Adjusts the efficiency of DX Cooling Coils in all rooftop units according to NECB cooling requirements.
6) Economizer and ERV Control
    * Activates or deactivates the economizer depending on the building and NECB requirements.
    * Activates or deactivates the ERV (Energy Recovery Ventilator) according to NECB guidelines.

Impact and Benefits

* Automation: Replaces manual iterative steps in reference model creation.
* Consistency: Ensures all parameters comply with NECB Part 8 across all zones and systems.
* Efficiency: Reduces model preparation time significantly.
* Error Prevention: Minimizes human error in configuring rooftop units, baseboards, DX coils, economizers, and ERVs.

This script is particularly useful for light commercial buildings with multiple rooftop units, where manual creation of a reference model can be tedious and error-prone. By modifying
the IDF directly, the script ensures that all required changes are accurately applied, and the model is ready for simulation immediately.
*/
